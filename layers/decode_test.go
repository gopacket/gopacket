...
var cdpTestData = map[string]struct {
	data []byte
	want *CiscoDiscoveryInfo
}{
	"cdp - IPv4 addresses only": {
	...
	"cdp - IPv4 and IPv6 addresses": {
		data: []byte{
			,
		},
		want: &CiscoDiscoveryInfo{
			// Example has no cluster management hello
			CDPHello: CDPHello{
				OUI:              nil,
				ProtocolID:       0,
				ClusterMaster:    nil,
				...
				Status:           0,
				Unknown2:         0,
				ClusterCommander: nil,
				SwitchMAC:        nil,
				Unknown3:         0,
				ManagementVLAN:   0,
			},
			DeviceID: "009ad24baaaa",
			Addresses: []net.IP{
				net.IPv4(192, 168, 50, 6),
				//        + 		s"2603:8080:e501:d301:29a:d2ff:fe4b:76a3",
				{0x26, 0x03, 0x80, 0x80, 0xe5, 0x01, 0xaa, 0xaa, 0x02, 0x9a, 0xd2, 0xff, 0xfe, 0xaa, 0xaa, 0xaa},
				{0xfe, 0x80, 0, 0, 0, 0, 0, 0, 0x02, 0x9a, 0xd2, 0xff, 0xfe, 0xaa, 0xaa, 0xaa},
			},
			PortID:        "gi1",
			Capabilities:  CDPCapabilities{false, false, false, true, false, true, false, false, false},
			Version:       "1.4.11.5",
			Platform:      "Cisco SG200-26 (PID:SLM2024T)-VSD",
			VTPDomain:     "",
			NativeVLAN:    1,
			FullDuplex:    true,
			MgmtAddresses: nil,
			SysName:       "sg200-26-dot6",
			PowerAvailable: CDPPowerDialogue{
				ID:     0,
				MgmtID: 31581,
				Values: []uint32{0, 4294967295},
			},
		},
	},
}
...
func TestDecodeCiscoDiscovery(t *testing.T) {
	for label, td := range cdpTestData {
		t.Run(label, func(t *testing.T) {
			td.want.BaseLayer = BaseLayer{Contents: td.data[26:]}
			p := gopacket.NewPacket(td.data, LinkTypeEthernet, testDecodeOptions)
			...
			if !reflect.DeepEqual(info, td.want) {
